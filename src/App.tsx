...
export default function App() {
  ...
  const initial: Aktivitet[] = [
    ...
    ...Array.from({ length: blankCount }, (_, i) => ({
      id: String(dummies.length + i + 1), navn: '', start: '', slutt: '', ansvarlig: '', status: ''
    }))
  ]
      ...
      <div className="card">
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          ...
          <div className="toolbar">
            ...
            <button className="btn" onClick={() => { setRows(initial); setQuick('') }}>Reset</button>
          </div>
        </div>
        <AktivitetAGGrid rows={rows} onRowsChange={setRows} quickFilterText={quick} />
      </div>
    </div>
  )
}
